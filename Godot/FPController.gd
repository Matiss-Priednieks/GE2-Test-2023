...
func _process(delta):	
	if move:
		...
		playerrotation.interpolate_property(self, "global_rotation", global_rotation, $"../creature/boid".global_transform.basis.rotated(Vector3(0, 1, 0), 160).get_euler(), 0.1, Tween.TRANS_CIRC, Tween.EASE_IN)
